...
void ObjReader::readObj(char* fileName, int &nVertices, double **vertices, double **normals, double **texcoords, int &nIndices, int **indices)
{
	...
	double *n = new double[3*(nn>nf?nn:nf)];
	double *v = new double[3*nv];
	double *t = new double[2*nt];
	...
	*vertices = new double[ntriangles*9];
	if(!noNormals) *normals = new double[ntriangles*9]; else *normals=0;
	if(!noTexCoords) *texcoords = new double[ntriangles*6]; else *texcoords=0;
