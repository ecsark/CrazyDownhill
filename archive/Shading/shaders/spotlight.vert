...
void main()
{
    ...
    float dist = length(gl_LightSource[2].position - gl_Position);
    vec4 lightDir = normalize(gl_LightSource[2].position - gl_Position);
    if (dot(lightDir.xyz,normal)>0.0) {
        gl_Position = gl_Position + (1000.0/(dist*dist))*lightDir;
    }
